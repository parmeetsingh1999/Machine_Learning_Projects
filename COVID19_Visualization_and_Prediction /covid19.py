...
latest_df = pd.read_csv('https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/10-22-2020.csv')
us_medical_data = pd.read_csv('https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports_us/10-22-2020.csv')
apple_mobility = pd.read_csv('https://covid19-static.cdn-apple.com/covid19-mobility-data/2019HotfixDev16/v3/en-us/applemobilitytrends-2020-10-22.csv')
print('Confirm case data -\n', confirmed_df.head())
print('Death case data -\n', deaths_df.head())
print('Recovery case data -\n', recoveries_df.head())
print('US medical data -\n', us_medical_data.head())
print('Apple mobility data -\n', apple_mobility.head())

for col in recoveries_df.columns: 
    print(col)
    
# Fetching the dates of the outbreak

confirmed = confirmed_df.loc[:, '1/22/20' : '10/23/20']
deaths = deaths_df.loc[:, '1/22/20' : '10/23/20']
recoveries = recoveries_df.loc[:, '1/22/20' : '10/23/20']
...
def moving_average(data, window_size):
    moving_average = []
    for i in range(len(data)):
        if i + window_size < len(data): moving_average.append(np.mean(data[i : i + window_size]))
        else: moving_average.append(np.mean(data[i : len(data)]))
...
#Slighty modify the data to fit the model better 
...
plt.xlabel('Dayes since 1/22/2020', size = 30)
plt.ylabel('# of cases', size = 30)
...
def country_plot(x, y1, y2, y3, y4, country):
    ...
    plt.legend(['{} confirmed cases'.format(country), 'Moving average {} days'.format(country)], prop = {'size': 20})
...
countries = ['US', 'Russia', 'India', 'Brazil', 'South Africa', 'China', 'Italy', 'Germany', 'Spain', 'France', 'United Kingdom', 'Peru', 'Mexico', 'Colombia', 'Saudi Arabia', 'Iran', 'Bangladesh', 'Pakistan', 'Turkey', 'Philippines', 'Iraq', 'Indonesia', 'Israel', 'Ukraine', 'Ecuador', 'Bolivia', 'Netherlands', 'Belgium'] 
for country in countries: country_visualizations(country)

#Country Comparison
compare_countries = ['USA', 'Brazil', 'India', 'Russia', 'South Africa']
graph_name = ['Coronavirus Confirmed Cases', 'Coronavirus Confirmed Deaths', 'Coronavirus Confirmed Recoveries']

for num in range(3):
    plt.figure(figsize = (16, 10))
    for country in compare_countries: plt.plot(get_country_info(country)[num])
...
linear_pred = pd.DataFrame({'Date': future_forecast_dates[-10:], 'Polynomial Predicted # of confirmed cases worldwide': np.round(linear_pred[-10:])})
linear_pred.style.background_gradient(cmap = 'Reds')

#Future prediction using Bayesian Ridge
...
plt.title('Worldwide recovery rate of coronavirus Ooer time', size = 30)
plt.xlabel('Days since 1/22/2020', size = 30)
...
for i in unique_countries:
    cases = latest_df[latest_df['Country_Region'] == i]['Confirmed'].sum()
    if cases > 0: country_confirmed_cases.append(cases)
    else: no_cases.append(i)
...
for i in range(len(unique_countries)):
    country_confirmed_cases[i] = latest_df[latest_df['Country_Region'] == unique_countries[i]]['Confirmed'].sum()
    country_death_cases.append(latest_df[latest_df['Country_Region'] == unique_countries[i]]['Deaths'].sum())
    country_recovery_cases.append(latest_df[latest_df['Country_Region'] == unique_countries[i]]['Recovered'].sum())
    country_active_cases.append(latest_df[latest_df['Country_Region'] == unique_countries[i]]['Active'].sum())
    country_incidence_rate.append(latest_df[latest_df['Country_Region'] == unique_countries[i]]['Incidence_Rate'].sum())
    country_mortality_rate.append(country_death_cases[i]/country_confirmed_cases[i])
    
# Data Table

country_df = pd.DataFrame({'Country Name': unique_countries, 'Number of confirmed cases': country_confirmed_cases, 'Number of deaths': country_death_cases, 'Number of recoveries': country_recovery_cases, 'Number of active cases': country_active_cases, 'Incidence rate': country_incidence_rate, 'Mortality rate': country_mortality_rate})
country_df.style.background_gradient(cmap='Oranges')

unique_provinces = list(latest_df['Province_State'].unique())

# Getting the latest information about provinces/sttes that have confirmed coronavirus cases
...
for i in unique_provinces:
    cases = latest_df[latest_df['Province_State'] == i]['Confirmed'].sum()
    if cases > 0: province_confirmed_cases.append(cases)
    else: no_cases.append(i)

#Remove areas with no confirmed cases
for i in no_cases: unique_provinces.remove(i)
    
unique_provinces = [k for k, v in sorted(zip(unique_provinces, province_confirmed_cases), key = operator.itemgetter(1), reverse = True)]
for i in range(len(unique_provinces)):
    province_confirmed_cases[i] = latest_df[latest_df['Province_State'] == unique_provinces[i]]['Confirmed'].sum()
    province_country.append(latest_df[latest_df['Province_State'] == unique_provinces[i]]['Country_Region'].unique()[0])
    province_death_cases.append(latest_df[latest_df['Province_State'] == unique_provinces[i]]['Deaths'].sum())
    province_active.append(latest_df[latest_df['Province_State'] == unique_provinces[i]]['Active'].sum())
    province_incidence_rate.append(latest_df[latest_df['Province_State'] == unique_provinces[i]]['Incidence_Rate'].sum())
...
def country_table(country_name):
    states = list(latest_df[latest_df['Country_Region'] == country_name]['Province_State'].unique())
    ...
    for i in states:
        cases = latest_df[latest_df['Province_State'] == i]['Confirmed'].sum()
        if cases > 0: state_confirmed_cases.append(cases)
        else: no_cases.append(i)

    #Remove areas with no confirmed cases
    for i in no_cases: states.remove(i)

    states = [k for k, v in sorted(zip(states, state_confirmed_cases), key = operator.itemgetter(1), reverse = True)]
    for i in range(len(states)):
        state_confirmed_cases[i] = latest_df[latest_df['Province_State'] == states[i]]['Confirmed'].sum()
        state_death_cases.append(latest_df[latest_df['Province_State'] == states[i]]['Deaths'].sum())
        state_active.append(latest_df[latest_df['Province_State'] == states[i]]['Active'].sum())
        state_incidence_rate.append(latest_df[latest_df['Province_State'] == states[i]]['Incidence_Rate'].sum())
        state_mortality_rate.append(state_death_cases[i] / state_confirmed_cases[i])
        
      
    state_df = pd.DataFrame({'State Name': states, 'Number of Confirmed Cases': state_confirmed_cases, 'Number of Deaths': state_death_cases, 'Number of Active Cases': state_active, 'Incidence Rate': state_incidence_rate, 'Mortality Rate': state_mortality_rate})
    return state_df

# Data table for United States
...
# Bar chart visualization for COVID 19

total_world_cases = np.sum(country_confirmed_cases)
us_confirmed = latest_df[latest_df['Country_Region'] == 'US']['Confirmed'].sum()
...
# Visual representations (bar and pie charts)
...
# Pie chart visualization for COVID 19
...
def plot_pie_country_with_regions(country_name, title):
    regions = list(latest_df[latest_df['Country_Region'] == country_name]['Province_State'].unique())
    confirmed_cases = []
    no_cases = [] 
    for i in regions:
        cases = latest_df[latest_df['Province_State'] == i]['Confirmed'].sum()
        if cases > 0: confirmed_cases.append(cases)
        else: no_cases.append(i)

    #Remove areas with no confirmed cases
    for i in no_cases: regions.remove(i)

    #Only show the top 5 states
    regions = [k for k, v in sorted(zip(regions, confirmed_cases), key = operator.itemgetter(1), reverse = True)]

    for i in range(len(regions)): confirmed_cases[i] = latest_df[latest_df['Province_State'] == regions[i]]['Confirmed'].sum()  
    ...
    else: plot_pie_charts(regions,confirmed_cases, title)
    
pie_chart_countries = ['US', 'Brazil', 'Russia', 'India', 'Peru', 'Mexico', 'Canada', 'Australia', 'China', 'Italy', 'Germany', 'France', 'United Kingdom', 'Chile', 'Colombia']

for i in pie_chart_countries: plot_pie_country_with_regions(i, 'Covid-19 Confirmed Cases in {}'.format(i))
...
# Peeking into Apple's Mobility data
...
def weekday_or_weekend(date):
    ...
    if (day_of_the_week + 1) % 6 == 0 or (day_of_the_week + 1) % 7 == 0: return True 
    else: return False 
    
revised_day_since_1_22 = [i for i in range(len(revised_dates))]

import matplotlib.dates as mdates
states = ['New York', 'Connecticut', 'Florida', 'California', 'Texas', 'Georgia', 'Arizona', 'Illinois', 'Louisiana', 'Ohio', 'Tennessee', 'North Carolina', 'South Carolina', 'Alabama', 'Missouri', 'Kansas', 'Pennsylvania', 'Wisconsin', 'Virginia', 'Massachusetts', 'Utah', 'Minnesota', 'Oklahoma', 'Iowa', 'Arkansas', 'Kentucky', 'Puerto Rico', 'Colorado', 'Idaho', 'New Jersey', 'Nevada', 'Maryland', 'Indiana', 'Washington', 'Montana', 'Rhode Island']
states.sort()
...
for state in states:
    ...
    for i in range(len(weekend_mobility)):       
        if weekend_mobility[i] == 0 and i != 0: weekend_mobility[i] = weekend_mobility[i - 1]
        elif weekend_mobility[i] == 0 and i == 0: weekend_mobility[i] = weekend_mobility[i + 1]
        else: pass
        
    for i in range(len(weekday_mobility)):       
        if weekday_mobility[i] == 0 and i != 0: weekday_mobility[i] = weekday_mobility[i - 1]
        elif weekday_mobility[i] == 0 and i == 0: weekday_mobility[i] = weekday_mobility[i + 1]
        else: pass
